...
losing control of quality, cost, or intent. Capability has largely been won;
operability has not. Gartner expects more than
...
<ul>
<li><strong>Control plane</strong> (platform repo): <code>platform.yml</code> (single source of truth) · <code>gnomes.yml</code> (fleet registry) · ADRs, guidance, handoffs · the <code>ledger</code> branch (run records, JSONL).</li>
...
Actions runs and interactive console sessions, which is what makes the ledger
and policy injection inescapable rather than aspirational.
